 ...
 * @internal
 */
class MarketplaceArchive {

	/**
	 * Register user item to menu
	 *
	 * @param \Elgg\Hook $hook 'register', 'menu:marketplace_archive'
	 *
	...
	public static function register(\Elgg\Hook $hook) {
		...
		$show_marketplace_archive = $hook->getParam('show_marketplace_archive') ?? in_array($page, ['all', 'owner', 'friends', 'group']);
		if (!$show_marketplace_archive) {
		...
		$options = array_merge($options, (array) $hook->getParam('marketplace_archive_options', []));
		...
		$marketplace_archive_url = $hook->getParam('marketplace_archive_url');
		$generate_url = function($lower = null, $upper = null) use ($page_owner, $page, $marketplace_archive_url) {
			if (!empty($marketplace_archive_url)) {
				$url_segment = elgg_http_add_url_query_elements($marketplace_archive_url, [
					'lower' => $lower,
					'upper' => $upper,
				]);
			} else if ($page_owner instanceof \ElggUser) {
				if ($page === 'friends') {
					$url_segment = elgg_generate_url('collection:object:marketplace:friends', [
						'username' => $page_owner->username,
						'lower' => $lower,
						'upper' => $upper,
					]);
				} else {
					$url_segment = elgg_generate_url('collection:object:marketplace:owner', [
						'username' => $page_owner->username,
						'lower' => $lower,
						'upper' => $upper,
					]);
				...
				$url_segment = elgg_generate_url('collection:object:marketplace:group', [
				...
				$url_segment = elgg_generate_url('collection:object:marketplace:all', [
					'lower' => $lower,
					'upper' => $upper,
				]);
			}
