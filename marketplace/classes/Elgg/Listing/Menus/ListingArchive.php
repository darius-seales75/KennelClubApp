...
class ListingArchive {
	 ...
	 *
	 * @param \Elgg\Hook $hook 'register', 'menu:listing_archive'
	 *
	...
	public static function register(\Elgg\Hook $hook) {
		...
		$show_listing_archive = $hook->getParam('show_listing_archive') ?? in_array($page, ['all', 'owner', 'friends', 'group']);
		if (!$show_listing_archive) {
		...
		$options = array_merge($options, (array) $hook->getParam('listing_archive_options', []));
		...
		$listing_archive_url = $hook->getParam('listing_archive_url');
		$generate_url = function($lower = null, $upper = null) use ($page_owner, $page, $listing_archive_url) {
			if (!empty($listing_archive_url)) {
				$url_segment = elgg_http_add_url_query_elements($listing_archive_url, [
					'lower' => $lower,
					'upper' => $upper,
				]);
			} else if ($page_owner instanceof \ElggUser) {
				if ($page === 'friends') {
					$url_segment = elgg_generate_url('collection:object:listing:friends', [
						'username' => $page_owner->username,
						'lower' => $lower,
						'upper' => $upper,
					]);
				} else {
					$url_segment = elgg_generate_url('collection:object:listing:owner', [
						'username' => $page_owner->username,
						'lower' => $lower,
						'upper' => $upper,
					]);
				...
				$url_segment = elgg_generate_url('collection:object:listing:group', [
				...
				$url_segment = elgg_generate_url('collection:object:listing:all', [
					'lower' => $lower,
					'upper' => $upper,
				]);
			}
