...
define(['jquery', 'elgg/Ajax', 'elgg/i18n'], function($, Ajax, i18n) {
	...
	var oldDescription = $('form.elgg-form-listing-save').find('textarea[name=description]').val();

	var saveDraftCallback = function(data) {
		var $form = $('form.elgg-form-listing-save');
		...
		if (data.success == true) {
			...
			$form.find('.listing-save-status-time').html(d.toLocaleDateString() + " @ " + d.getHours() + ":" + mins);
		} else {
			$form.find('.listing-save-status-time').html(i18n.echo('error'));
		}
	};
	...
	var saveDraft = function() {
		...
		var formData = ajax.objectify('form.elgg-form-listing-save');
		...
		ajax.action('listing/auto_save_revision', {
			data: formData,
	...
	$(document).on('click', '.elgg-form-listing-save button[name="preview"]', function(event) {
		...
		ajax.action('listing/save', {
			data: formData,
			success: function(data) {
				$('form.elgg-form-listing-save').find('input[name=guid]').val(data.guid);
